...
use azul_core::{ui_description::UiDescription, ui_solver::ScrolledNodes, window::WindowId};
...
#[cfg(not(test))]
use webrender::api::{HitTestFlags, WorldPoint};
use webrender::{
...
use crate::{
    ...
    gl::GlShader,
    r#async::{Task, TerminateTimer, TimerId},
    traits::Layout,
    ui_state::UiState,
    window::{
        DebugState, FullWindowState, LogicalPosition, LogicalSize, RendererType, ScrollStates,
        Window, WindowSize,
    },
    FastHashMap,
...
const COLOR_WHITE: ColorU = ColorU {
    r: 255,
    g: 255,
    b: 255,
    a: 0,
...
impl<T: 'static> App<T> {
    ...
    #[cfg(not(test))]
    pub fn create_hot_reload_window(
        &mut self,
        options: WindowCreateOptions,
...
fn initialize_ui_description_cache<T>(
    ...
    ui_states
        .iter()
        .map(|(window_id, ui_states)| {
            (
                *window_id,
                ui_states
                    .iter()
                    .map(|(dom_id, _)| (dom_id.clone(), UiDescription::default()))
                    .collect(),
            )
        })
        .collect()
...
fn hit_test_single_window<T>(
    ...
    let scroll_states = window
        .internal
        .get_current_scroll_states(&ui_state_cache[window_id]);
    ...
    ret.hit_test_results = ret
        .hit_test_results
        .or_else(|| do_hit_test(window, full_window_state, fake_display));
    ...
    for (dom_id, callback_scrolled_nodes) in scrolled_nodes {
        if let Some(scrolled_nodes) = window.internal.scrolled_nodes.get(&dom_id) {
            for (scroll_node_id, scroll_position) in &callback_scrolled_nodes {
                if let Some(overflowing_node) =
                    scrolled_nodes.overflowing_nodes.get(&scroll_node_id)
                {
                    window
                        .internal
                        .scroll_states
                        .set_scroll_position(&overflowing_node, *scroll_position);
    ...
    let should_scroll_render =
        should_scroll_render_from_input_events || should_scroll_render_from_callbacks;
...
fn relayout_single_window<T>(
...

    {
        ...
        let ui_description_mut = ui_description_cache
            .get_mut(window_id)
            .ok_or(WindowIndexError)?;
        *ui_description_mut = ui_state_mut
            .iter_mut()
            .map(|(dom_id, ui_state)| {
...
fn do_hit_test<T>(
    ...
    let cursor_location = full_window_state
        ...
        .cursor_pos
        .get_position()
        .map(|pos| WorldPoint::new(pos.x, pos.y))?;

    let mut hit_test_results: Vec<HitTestItem> = fake_display
        .render_api
        .hit_test(
...
fn call_callbacks<T>(
    ...
    let hit_test_items = hit_test_results
        .map(|items| items.clone())
        .unwrap_or_default();
    let callbacks_filter_list = ui_state_map
        .iter()
        .map(|(dom_id, ui_state)| {
            (
                dom_id.clone(),
                determine_callbacks(full_window_state, &hit_test_items, event, ui_state),
            )
        })
    ...
    for dom_id in ui_state_map.keys().cloned() {
        for (node_id, callback_results) in
            callbacks_filter_list[&dom_id].nodes_with_callbacks.iter()
        {
            ...
            for default_callback_id in callback_results.default_callbacks.values() {
                ...
                if app_state.windows[window_id]
                    .default_callbacks
                    .get(default_callback_id)
                    .cloned()
                    .and_then(|(callback_ptr, callback_fn)| {
                        let info = DefaultCallbackInfoUnchecked {
                            ...
                            cursor_relative_to_item: hit_item.as_ref().map(|hi| {
                                (hi.point_relative_to_item.x, hi.point_relative_to_item.y)
                            }),
                            cursor_in_viewport: hit_item
                                .as_ref()
                                .map(|hi| (hi.point_in_viewport.x, hi.point_in_viewport.y)),
                        };
                        (callback_fn.0)(info)
                    })
                    == Redraw
            ...
            callbacks_filter_list[&dom_id].nodes_with_callbacks.iter()
        {
            ...
            for callback in callback_results.normal_callbacks.values() {
                ...
                if (callback.0)(CallbackInfo {
                    ...
                    ui_state: ui_state_map,
                    hit_test_items: &hit_test_items,
                    cursor_relative_to_item: hit_item
                        .as_ref()
                        .map(|hi| (hi.point_relative_to_item.x, hi.point_relative_to_item.y)),
                    cursor_in_viewport: hit_item
                        .as_ref()
                        .map(|hi| (hi.point_in_viewport.x, hi.point_in_viewport.y)),
                }) == Redraw
    ...
    Ok(CallCallbackReturn {
        ...
        needs_redraw_anyways: callbacks_filter_list
            .values()
            .any(|v| v.needs_redraw_anyways),
        needs_relayout_anyways: callbacks_filter_list
            .values()
            .any(|v| v.needs_relayout_anyways),
    })
}
...
fn update_display_list<T>(
    ...
    unsafe { fake_display.make_current() };
...
fn update_scroll_state(
    ...
    for scroll_node in hit_test_items
        .iter()
        .filter_map(|item| {
            scrolled_nodes
                .tags_to_node_ids
...
fn render_inner<T>(
    ...
    txn.set_root_pipeline(wr_translate::wr_translate_pipeline_id(
    ...
    unsafe {
        ...
        gl_context.clear_color(
            background_color_f.r,
            background_color_f.g,
            background_color_f.b,
            background_color_f.a,
        );
    ...
    if window.create_options.state.is_visible && window.state.is_visible {
        window.display.gl_window().window().set_visible(true);
...
fn set_webrender_debug_flags(r: &mut Renderer, old_flags: &DebugState, new_flags: &DebugState) {
    ...
    if old_flags.profiler_dbg != new_flags.profiler_dbg {
        flags.set(DebugFlags::PROFILER_DBG, new_flags.profiler_dbg);
    }
    if old_flags.render_target_dbg != new_flags.render_target_dbg {
        flags.set(DebugFlags::RENDER_TARGET_DBG, new_flags.render_target_dbg);
    }
    if old_flags.texture_cache_dbg != new_flags.texture_cache_dbg {
        flags.set(DebugFlags::TEXTURE_CACHE_DBG, new_flags.texture_cache_dbg);
    }
    if old_flags.gpu_time_queries != new_flags.gpu_time_queries {
        flags.set(DebugFlags::GPU_TIME_QUERIES, new_flags.gpu_time_queries);
    }
    if old_flags.gpu_sample_queries != new_flags.gpu_sample_queries {
        flags.set(DebugFlags::GPU_SAMPLE_QUERIES, new_flags.gpu_sample_queries);
    }
    if old_flags.disable_batching != new_flags.disable_batching {
        flags.set(DebugFlags::DISABLE_BATCHING, new_flags.disable_batching);
    }
    if old_flags.epochs != new_flags.epochs {
        flags.set(DebugFlags::EPOCHS, new_flags.epochs);
    }
    if old_flags.compact_profiler != new_flags.compact_profiler {
        flags.set(DebugFlags::COMPACT_PROFILER, new_flags.compact_profiler);
    }
    ...
    if old_flags.show_overdraw != new_flags.show_overdraw {
        flags.set(DebugFlags::SHOW_OVERDRAW, new_flags.show_overdraw);
    }
    if old_flags.gpu_cache_dbg != new_flags.gpu_cache_dbg {
        flags.set(DebugFlags::GPU_CACHE_DBG, new_flags.gpu_cache_dbg);
    }
