...
pub trait Modify<T> {
    ...
    fn modify_opt<F>(&self, closure: F) -> Option<()>
    where
        F: FnOnce(&mut T) -> Option<()>,
    {
    ...
    fn modify_opt_clone<F, S>(&self, closure: F) -> Option<S>
    where
        F: FnOnce(&mut T) -> Option<S>,
    {
        let mut initial: Option<S> = None;
