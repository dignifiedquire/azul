...
use harfbuzz_sys::{
    hb_blob_create, hb_blob_destroy, hb_blob_t, hb_buffer_add_utf8,
    hb_buffer_allocation_successful, hb_buffer_create, hb_buffer_destroy,
    hb_buffer_get_glyph_infos, hb_buffer_get_glyph_positions, hb_buffer_guess_segment_properties,
    hb_buffer_t, hb_face_create, hb_face_destroy, hb_face_t, hb_feature_t, hb_font_create,
    hb_font_destroy, hb_font_set_scale, hb_font_t, hb_glyph_info_t, hb_glyph_position_t,
    hb_memory_mode_t, hb_ot_font_set_funcs, hb_shape, hb_tag_t, HB_MEMORY_MODE_READONLY,
};
use std::{
    ops::Deref,
...
const FEATURE_KERNING_OFF: hb_feature_t = hb_feature_t {
    tag: KERN_TAG,
    value: 0,
    start: 0,
    end: u32::MAX,
};
const FEATURE_KERNING_ON: hb_feature_t = hb_feature_t {
    tag: KERN_TAG,
    value: 1,
    start: 0,
    end: u32::MAX,
};
const FEATURE_LIGATURE_OFF: hb_feature_t = hb_feature_t {
    tag: LIGA_TAG,
    value: 0,
    start: 0,
    end: u32::MAX,
};
const FEATURE_LIGATURE_ON: hb_feature_t = hb_feature_t {
    tag: LIGA_TAG,
    value: 1,
    start: 0,
    end: u32::MAX,
};
const FEATURE_CLIG_OFF: hb_feature_t = hb_feature_t {
    tag: CLIG_TAG,
    value: 0,
    start: 0,
    end: u32::MAX,
};
const FEATURE_CLIG_ON: hb_feature_t = hb_feature_t {
    tag: CLIG_TAG,
    value: 1,
    start: 0,
    end: u32::MAX,
...
static ACTIVE_HB_FEATURES: [hb_feature_t; 3] =
    [FEATURE_KERNING_ON, FEATURE_LIGATURE_ON, FEATURE_CLIG_ON];
...
pub(crate) fn shape_word_hb<'a>(
    ...
    let glyph_positions =
        unsafe { hb_buffer_get_glyph_positions(text.hb_buffer, &mut position_count) };
...
pub(crate) fn get_word_visual_width_hb(glyph_positions: &[GlyphPosition]) -> f32 {
    glyph_positions
        .iter()
        .map(|pos| pos.x_advance as f32 / HB_SCALE_FACTOR)
        .sum()
}
