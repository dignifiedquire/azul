...
impl<T> XmlComponent<T> for DynamicXmlComponent {
    ...
    fn render_dom(
        ...
        for child_node in &self.root.children {
            dom.add_child(render_dom_from_app_node_inner(
...
impl<T> XmlComponentMap<T> {
    pub fn register_component<S: AsRef<str>>(
        &mut self,
        id: S,
        component: Box<dyn XmlComponent<T>>,
        inherit_variables: bool,
    ) {
        self.components.insert(
            normalize_casing(id.as_ref()),
            (component, inherit_variables),
        );
    }
    pub fn register_callback<S: AsRef<str>>(&mut self, id: S, callback: Callback<T>) {
        self.callbacks
            .insert(normalize_casing(id.as_ref()), callback);
    }
}
...
impl fmt::Display for XmlParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            MultipleRootComponents => write!(
                f,
                "Multiple <app/> components present, only one root node is allowed"
            ),
            ParseError(e) => write!(f, "XML parsing error: {}", e),
            MalformedHierarchy(got, expected) => {
                write!(f, "Invalid </{}> tag: expected </{}>", got, expected)
            }
...
impl fmt::Display for ComponentParseError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            ...
            WhiteSpaceInComponentName(arg_pos, arg_name_unparsed) => write!(
                f,
                "Missing `:` between the name and the type in argument {} (around \"{}\")",
                arg_pos, arg_name_unparsed
            ),
            WhiteSpaceInComponentType(arg_pos, arg_name, arg_type_unparsed) => write!(
                f,
...
impl fmt::Display for RenderDomError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            UselessFunctionArgument(k, v, available_args) => write!(
                f,
        ...
        match token {
            ...
            Attribute((_, key), value) => {
                if let Some(last) = get_item(&current_hierarchy, &mut root_node) {
                    ...
                    last.attributes
                        .insert(normalize_casing(key.to_str()), value.to_str().to_string());
                }
            }
            Text(t) => {
                if let Some(last) = get_item(&current_hierarchy, &mut root_node) {
...
fn validate_and_filter_component_args(
    xml_attributes: &XmlAttributeMap,
    valid_args: &FilteredComponentArguments,
...
fn get_app_node(root_nodes: &[XmlNode]) -> Result<XmlNode, XmlParseError> {
    let mut app_node_iterator = root_nodes
        .iter()
        .filter(|node| {
    ...
    let app_node = app_node_iterator
        .next()
        .ok_or(XmlParseError::NoRootComponent)?;
...
fn get_xml_components<T>(
    root_nodes: &[XmlNode],
    components: &mut XmlComponentMap<T>,
) -> Result<(), ComponentParseError> {
    for node in root_nodes {
        match DynamicXmlComponent::new(node.clone()) {
            Ok(node) => {
                components.register_component(node.name.clone(), Box::new(node), false);
            }
...
fn compile_components(
    components: BTreeMap<ComponentName, (CompiledComponent, FilteredComponentArguments)>,
) -> String {
    components
        .iter()
        .map(|(name, (function_body, function_args))| {
...
fn render_dom_from_app_node_inner<T>(
    ...
    let (renderer, inherit_variables) = component_map
        .components
        .get(&component_name)
    ...
    let text = xml_node
        .text
        .as_ref()
        .map(|t| format_args_dynamic(t, &filtered_xml_attributes));
...
fn set_attributes<T>(
    ...
    if let Some(ids) = xml_attributes.get("id") {
        for id in ids.split_whitespace() {
            dom.add_id(DomString::Heap(format_args_dynamic(
                id,
                &filtered_xml_attributes,
            )));
        }
    ...
    if let Some(classes) = xml_attributes.get("class") {
        for class in classes.split_whitespace() {
            dom.add_class(DomString::Heap(format_args_dynamic(
                class,
                &filtered_xml_attributes,
            )));
        }
    ...
    if let Some(drag) = xml_attributes
        .get("draggable")
                    ...
                    match opening_braces.pop() {
                        Some(last_open) => {
                            let variable_name: String =
                                input[(last_open + 1)..ch_idx].iter().collect();
...
fn compile_components_to_rust_code<T>(
    components: &XmlComponentMap<T>,
...
fn compile_app_node_to_rust_code<T>(
    app_node: &XmlNode,
    component_map: &XmlComponentMap<T>,
) -> Result<String, CompileError> {
...
fn compile_app_node_to_rust_code_inner<T>(
    app_node: &XmlNode,
    component_map: &XmlComponentMap<T>,
) -> Result<String, CompileError> {
    // TODO!
