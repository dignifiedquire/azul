use azul_core::{
    ...
    ui_solver::{InlineTextLayout, LayoutResult, PositionedRectangle, ResolvedTextLayoutOptions},
};
use azul_css::{
    LayoutRect, LayoutSize, RectLayout, RectStyle, StyleFontSize, StyleTextAlignmentHorz,
...
use {
    ...
    text_layout::{LayoutedGlyphs, ScaledWords, WordPositions, Words},
};
...
pub(crate) enum PreferredHeight {
    Image {
        original_dimensions: (usize, usize),
        aspect_ratio: f32,
...
pub(crate) fn px_to_au(px: f32) -> Au {
    use app_units::{Au as WrAu, AU_PER_PX, MAX_AU, MIN_AU};
...
pub(crate) fn get_font_id(rect_style: &RectStyle) -> &str {
    ...
    let font_id = rect_style
        .font_family
        .as_ref()
        .and_then(|family| family.get_property()?.fonts.get(0));
...
pub(crate) fn do_the_layout<'a, 'b, T>(
    ...
    let mut solved_ui = {
        let rect_contents =
            create_rect_contents_cache(&word_cache, &scaled_words, node_data, app_resources);
    ...
    let positioned_word_cache =
        create_word_positions(&word_cache, &scaled_words, &solved_ui.solved_rects);
    let layouted_glyph_cache = get_glyphs(
        node_hierarchy,
        &scaled_words,
        &positioned_word_cache,
        &display_rects,
        &mut solved_ui.solved_rects,
    );
...
fn create_word_cache<T>(
    ...
    node_data
        ...
        .filter_map(|node_id| match &node_data[node_id].get_node_type() {
            ...
            NodeType::Text(text_id) => app_resources
                .get_text(text_id)
                .map(|words| (node_id, words.clone())),
            _ => None,
        })
        .collect()
}
...
fn create_rect_contents_cache<'a, T>(
    ...
    display_rects
        .linear_iter()
        .filter_map(|node_id| match *display_rects[node_id].get_node_type() {
            Image(id) => {
...
fn create_word_positions<'a>(
    ...
    words
        ...
        .filter_map(|(node_id, words)| {
            ...
            let (text_layout_options, _, _) = layouted_rects[*node_id]
                .resolved_text_layout_options
                .as_ref()?;
            let positioned_words =
...
fn get_glyphs<'a>(
    ...
    scaled_words
        ...
        .filter_map(|(node_id, (scaled_words, _))| {
            ...
            let (horz_alignment, vert_alignment) =
                determine_text_alignment(&display_rect.style, &display_rect.layout);
            ...
            let (_, inline_text_layout, _) = positioned_rectangles[*node_id]
                .resolved_text_layout_options
                .as_mut()?;
            inline_text_layout.align_children_horizontal(horz_alignment);
            inline_text_layout
                .align_children_vertical_in_parent_bounds(&parent_bounds, vert_alignment);
...
fn determine_text_alignment(
    ...
    if let Some(align_items) = rect_layout
        .align_items
        .and_then(|ai| ai.get_property_or_default())
    {
    ...
    if let Some(justify_content) = rect_layout
        .justify_content
        .and_then(|jc| jc.get_property_or_default())
    {
    ...
    if let Some(text_align) = rect_style
        .text_align
        .and_then(|ta| ta.get_property_or_default())
    {
