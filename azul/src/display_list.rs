use azul_core::{
    app_resources::{FontInstanceKey, ImageId},
    callbacks::PipelineId,
    display_list::{
        AlphaType, CachedDisplayList, DisplayListFrame, DisplayListMsg, DisplayListScrollFrame,
        ImageRendering, LayoutRectContent, StyleBorderColors, StyleBorderRadius, StyleBorderStyles,
        StyleBorderWidths, StyleBoxShadow,
    },
    ui_solver::{
        ExternalScrollId, LayoutResult, OverflowingScrollNode, PositionedRectangle,
        ResolvedOffsets, ScrolledNodes,
    },
};
use azul_css::{
    BoxShadowClipMode, ColorU, Css, CssProperty, CssPropertyValue, LayoutPoint, LayoutPosition,
    LayoutRect, LayoutSize, RectLayout, RectStyle,
};
use azul_layout::{style::Style, GetStyle};
...
use {
    ...
    dom::{
        DomId, DomString, NodeData,
        NodeType::{self, Div, GlTexture, IFrame, Image, Label, Text},
        ScrollTagId,
    },
    id_tree::{NodeDataContainer, NodeHierarchy, NodeId},
    text_layout::LayoutedGlyphs,
    ui_description::{StyledNode, UiDescription},
    ui_solver::do_the_layout,
    ui_state::UiState,
    window::{FakeWindow, Window, WindowSize},
    FastHashMap,
};
...
impl<'a> GetStyle for DisplayRectangle<'a> {
    fn get_style(&self) -> Style {
        ...
        use azul_css::{
            LayoutAlignContent, LayoutAlignItems, LayoutBoxSizing, LayoutDirection, LayoutDisplay,
            LayoutJustifyContent, LayoutWrap, Overflow as LayoutOverflow, PixelValue,
        };
        use azul_layout::{style::*, Number, Offsets, Size};
        ...
        Style {
            ...
            position_type: match rect_layout
                .position
                .unwrap_or_default()
                .get_property_or_default()
            {
            ...
            flex_direction: match rect_layout
                .direction
                .unwrap_or_default()
                .get_property_or_default()
            {
            ...
            align_content: match rect_layout
                .align_content
                .unwrap_or_default()
                .get_property_or_default()
            {
            ...
            justify_content: match rect_layout
                .justify_content
                .unwrap_or_default()
                .get_property_or_default()
            {
            ...
            position: Offsets {
                ...
                right: translate_dimension(
                    rect_layout.right.map(|prop| prop.map_property(|r| r.0)),
                ),
                top: translate_dimension(rect_layout.top.map(|prop| prop.map_property(|t| t.0))),
                bottom: translate_dimension(
                    rect_layout.bottom.map(|prop| prop.map_property(|b| b.0)),
                ),
            },
            margin: Offsets {
                left: translate_dimension(
                    rect_layout
                        .margin_left
                        .map(|prop| prop.map_property(|l| l.0)),
                ),
                right: translate_dimension(
                    rect_layout
                        .margin_right
                        .map(|prop| prop.map_property(|r| r.0)),
                ),
                top: translate_dimension(
                    rect_layout
                        .margin_top
                        .map(|prop| prop.map_property(|t| t.0)),
                ),
                bottom: translate_dimension(
                    rect_layout
                        .margin_bottom
                        .map(|prop| prop.map_property(|b| b.0)),
                ),
            },
            padding: Offsets {
                left: translate_dimension(
            ...
            flex_shrink: rect_layout
                ...
                .get(),
            size: Size {
                width: translate_dimension(
                    rect_layout.width.map(|prop| prop.map_property(|l| l.0)),
                ),
                height: translate_dimension(
                    rect_layout.height.map(|prop| prop.map_property(|l| l.0)),
                ),
            },
            min_size: Size {
                width: translate_dimension(
                    rect_layout.min_width.map(|prop| prop.map_property(|l| l.0)),
                ),
                height: translate_dimension(
                    rect_layout
                        .min_height
            ...
            max_size: Size {
                width: translate_dimension(
                    rect_layout.max_width.map(|prop| prop.map_property(|l| l.0)),
                ),
                height: translate_dimension(
                    rect_layout
                        .max_height
            ...
            font_size_px: rect_style
                .font_size
                .and_then(|fs| fs.get_property_owned())
                .unwrap_or(DEFAULT_FONT_SIZE)
                .0,
            line_height: rect_style
                .line_height
                .and_then(|lh| lh.map_property(|lh| lh.0).get_property_owned())
                .map(|lh| lh.get()),
            letter_spacing: rect_style
                .letter_spacing
                .and_then(|ls| ls.map_property(|ls| ls.0).get_property_owned()),
            word_spacing: rect_style
                .word_spacing
                .and_then(|ws| ws.map_property(|ws| ws.0).get_property_owned()),
            tab_width: rect_style
                .tab_width
                .and_then(|tw| tw.map_property(|tw| tw.0).get_property_owned())
                .map(|tw| tw.get()),
        }
    }
}
...
fn determine_rendering_order<'a>(
    ...
    let children_sorted: BTreeMap<NodeId, Vec<NodeId>> = node_hierarchy
        ...
        .map(|(_, parent_id)| {
            (
                parent_id,
                sort_children_by_position(parent_id, node_hierarchy, rectangles),
            )
        })
        .collect();
...
pub(crate) fn display_list_from_ui_description<'a, T>(
    ...
    let display_rect_arena = arena.node_data.transform(|_, node_id| {
        ...
        override_warnings.append(&mut populate_css_properties(
            &mut rect,
            node_id,
            &ui_description.dynamic_css_overrides,
        ));
        rect
    });
    ...
    #[cfg(feature = "logging")]
    {
        for warning in override_warnings {
            error!(
                ...
                warning.default.get_type(),
                warning.overridden_property,
            )
        }
    }
...
pub(crate) fn display_list_to_cached_display_list<'a, T, U: FontImageApi>(
    ...
    let layout_result = do_the_layout(
        ...
        LayoutRect {
            ...
            size: LayoutSize::new(
                window.state.size.dimensions.width,
                window.state.size.dimensions.height,
            ),
        },
    );

    let rects_in_rendering_order =
        determine_rendering_order(node_hierarchy, &display_list.rectangles);
...
fn displaylist_handle_rect<'a, 'b, 'c, 'd, 'e, 'f, T, U: FontImageApi>(
    ...
    let tag_id = rect.tag.map(|tag| (tag, 0)).or({
        referenced_mutable_content.scrollable_nodes[dom_id]
            .overflowing_nodes
            .get(&rect_idx)
        ...
        .and_then(|br| br.get_property())
    {
        ...
        fn get_image_info(
            ...
            style_image_id: &CssImageId,
        ) -> Option<RectBackground> {
        ...
        let background_content = match bg {
            ...
            Image(style_image_id) => {
                get_image_info(referenced_mutable_content.app_resources, style_image_id)
            }
        ...
        if let Some(background_content) = background_content {
            frame.content.push(LayoutRectContent::Background {
                ...
                size: rect
                    .style
                    .background_size
                    .and_then(|bs| bs.get_property().cloned()),
                offset: rect
                    .style
                    .background_position
                    .and_then(|bs| bs.get_property().cloned()),
    ...
    match html_node {
        Div => {}
        Text(_) | Label(_) => {
            if let Some(layouted_glyphs) = referenced_mutable_content.layout_result[dom_id]
                ...
                .cloned()
            {
                ...
                let text_color = rect
                    .style
                    .text_color
                    .and_then(|tc| tc.get_property().cloned())
                    .unwrap_or(DEFAULT_FONT_COLOR)
                    .0;
                let positioned_words = &referenced_mutable_content.layout_result[dom_id]
                    .positioned_word_cache[&rect_idx];
        ...
        Image(image_id) => {
            if let Some(image_info) = referenced_mutable_content
                .app_resources
        ...
        GlTexture(callback) => {
            frame.content.push(call_opengl_callback(
                callback,
                bounds,
                dom_id.clone(),
                rectangle,
        ...
        IFrame(callback) => {
            ...
            frame.children.push(call_iframe_callback(
                callback,
                bounds,
                rectangle,
                referenced_content,
    ...
    match referenced_mutable_content.scrollable_nodes[dom_id]
        .overflowing_nodes
        .get(&rect_idx)
    {
...
fn call_opengl_callback<'a, 'b, 'c, 'd, 'e, 'f, T, U: FontImageApi>(
    ...
    let texture = match texture {
        Some(s) => s,
        None => {
            return LayoutRectContent::Background {
...
fn call_iframe_callback<'a, 'b, 'c, 'd, 'e, T, U: FontImageApi>(
    ...
    let scrollable_nodes_iframe = get_nodes_that_need_scroll_clip(
        ...
        &display_list.rectangles,
        node_data,
        &layout_result_iframe.rects,
        &layout_result_iframe.node_depths,
    ...
    referenced_mutable_content
        .scrollable_nodes
        .insert(iframe_dom_id.clone(), scrollable_nodes_iframe);
    referenced_mutable_content
        .layout_result
        .insert(iframe_dom_id.clone(), layout_result_iframe);
    ...
    css_constraints
        ...
        .filter_map(|constraint| match constraint {
            ...
            Dynamic(dynamic_property) => {
                let overridden_property = css_overrides.get(&node_id).and_then(|overrides| {
                    overrides.get(&dynamic_property.dynamic_id.clone().into())
                })?;

                // Apply the property default if the discriminant of the two types matches
                if mem::discriminant(overridden_property)
                    == mem::discriminant(&dynamic_property.default_value)
                {
