...
use azul_core::{
    app_resources::{
        Au, FontInstanceKey, FontKey, IdNamespace, ImageDescriptor, ImageKey,
        RawImageFormat as ImageFormat,
    },
    callbacks::{HidpiAdjustedBounds, HitTestItem, PipelineId},
    display_list::{
        AlphaType, CachedDisplayList, DisplayListFrame, DisplayListMsg, DisplayListScrollFrame,
        FontInstanceFlags, FontRenderMode, GlyphInstance, GlyphOptions, ImageRendering,
        LayoutRectContent, StyleBorderRadius,
    },
...
use azul_css::{
    ...
    ExtendMode as CssExtendMode, LayoutPoint, LayoutPoint as CssLayoutPoint, LayoutRect,
...
use webrender::api::{
    ...
    BuiltDisplayList as WrBuiltDisplayList, ColorF as WrColorF, ColorU as WrColorU,
    ...
    FontInstanceKey as WrFontInstanceKey, FontKey as WrFontKey, FontRenderMode as WrFontRenderMode,
    GlyphInstance as WrGlyphInstance, GlyphOptions as WrGlyphOptions, HitTestItem as WrHitTestItem,
    IdNamespace as WrIdNamespace, ImageDescriptor as WrImageDescriptor,
    ImageFormat as WrImageFormat, ImageKey as WrImageKey, ImageRendering as WrImageRendering,
    LayoutPoint as WrLayoutPoint, LayoutPrimitiveInfo as WrLayoutPrimitiveInfo,
    LayoutRect as WrLayoutRect, LayoutSideOffsets as WrLayoutSideOffsets,
    LayoutSize as WrLayoutSize, PipelineId as WrPipelineId, SpaceAndClipInfo as WrSpaceAndClipInfo,
};
...
pub(crate) fn wr_translate_hittest_item(input: WrHitTestItem) -> HitTestItem {
    HitTestItem {
        ...
        point_relative_to_item: LayoutPoint::new(
            input.point_relative_to_item.x,
            input.point_relative_to_item.y,
        ),
    }
}

#[inline(always)]
pub(crate) fn hidpi_rect_from_bounds(
    bounds: CssLayoutRect,
    hidpi_factor: f32,
...
pub(crate) fn wr_translate_image_descriptor(descriptor: ImageDescriptor) -> WrImageDescriptor {
    ...
    WrImageDescriptor {
        ...
        size: DeviceIntSize::new(
            descriptor.dimensions.0 as i32,
            descriptor.dimensions.1 as i32,
        ),
        stride: descriptor.stride,
        offset: descriptor.offset,
        is_opaque: descriptor.is_opaque,
...
fn translate_layout_rect_wr(input: WrLayoutRect) -> LayoutRect {
    LayoutRect::new(
        translate_layout_point_wr(input.origin),
...
fn push_display_list_msg(
    ...
    msg: DisplayListMsg,
    space_and_clip: &WrSpaceAndClipInfo,
) {
    use azul_core::display_list::DisplayListMsg::*;
    match msg {
        Frame(f) => push_frame(builder, f, space_and_clip),
        ScrollFrame(sf) => push_scroll_frame(builder, sf, space_and_clip),
    }
}
...
fn push_frame(
    ...
    let content_clip_id = builder.define_clip(
        root_space_and_clip,
        wr_rect,
        vec![content_clip],
        /* image_mask: */ None,
    ...
    let overflow_space_and_clip = match frame.clip_rect {
        Some(clip_rect) => {
        ...
        None => WrSpaceAndClipInfo::root_scroll(builder.pipeline_id),
    };
...
fn push_scroll_frame(
    ...
    let wr_border_radius = wr_translate_border_radius(
        ...
        scroll_frame.frame.rect.size,
    );

    let scroll_frame_clip_region =
        WrComplexClipRegion::new(wr_rect, wr_border_radius, WrClipMode::Clip);

    let hit_test_info = WrLayoutPrimitiveInfo {
        rect: wr_rect,
        clip_rect: wr_translate_layout_rect(
            scroll_frame
                .frame
                .clip_rect
                .unwrap_or(scroll_frame.frame.rect),
        ),
        is_backface_visible: false,
    ...
    let hit_testing_clip_id = builder.define_clip(
        root_space_and_clip,
        wr_rect,
        vec![scroll_frame_clip_region],
        None,
    );

    let hit_testing_space_and_clip = WrSpaceAndClipInfo {
        spatial_id: root_space_and_clip.spatial_id,
        clip_id: hit_testing_clip_id,
    };

    let scroll_frame_space_and_clip = builder.define_scroll_frame(
        &hit_testing_space_and_clip,
    ...
    builder.push_rect(
        &hit_test_info,
        &hit_testing_space_and_clip,
        wr_translate_color_u(ColorU::TRANSPARENT).into(),
    ); // push hit-testing rect

    // only children should scroll, not the frame itself
    for child in scroll_frame.frame.children {
        push_display_list_msg(builder, child, &scroll_frame_space_and_clip);
    }
}
...
fn push_display_list_content(
    ...
    match content {
        ...
        Border {
            ...
            border::push_border(builder, info, space_and_clip, radii, widths, colors, styles);
        }
        BoxShadow { shadow, clip_mode } => {
            box_shadow::push_box_shadow(
                builder,
                space_and_clip,
                translate_layout_rect_wr(info.rect),
                clip_mode,
                shadow,
                radii,
            );
...
mod text {
    ...
    use azul_core::{
        ...
        display_list::{GlyphInstance, GlyphOptions},
    };
    use azul_css::{ColorU, LayoutRect};
    use webrender::api::{
        DisplayListBuilder as WrDisplayListBuilder, LayoutPrimitiveInfo as WrLayoutPrimitiveInfo,
    ...
    pub(super) fn push_text(
        ...
        use super::{
            ...
            wr_translate_layout_size, wr_translate_layouted_glyphs,
        };
...
mod background {
    ...
    use super::image;
    use azul_core::{app_resources::ImageInfo, display_list::RectBackground};
    use azul_css::{
        ColorU, LayoutPoint, LayoutSize, LinearGradient, RadialGradient, StyleBackgroundPosition,
        StyleBackgroundRepeat, StyleBackgroundSize,
    };
    use webrender::api::{
        DisplayListBuilder as WrDisplayListBuilder, GradientStop as WrGradientStop,
        LayoutPrimitiveInfo as WrLayoutPrimitiveInfo, LayoutRect as WrLayoutRect,
        LayoutSize as WrLayoutSize, SpaceAndClipInfo as WrSpaceAndClipInfo,
    };
    ...
    pub(super) fn push_background(
        ...
        match background {
            ...
            Image(image_info) => push_image_background(
                builder,
                info,
                space_and_clip,
                image_info,
    ...
    fn push_radial_gradient_background(
        ...
        use super::{wr_translate_color_u, wr_translate_extend_mode, wr_translate_layout_size};
        use azul_css::Shape;
        ...
        let radius = match radial_gradient.shape {
            Shape::Ellipse => {
                WrLayoutSize::new(background_size.width / 2.0, background_size.height / 2.0)
            }
            Shape::Circle => {
    ...
    fn push_linear_gradient_background(
        ...
        use super::{
            ...
            wr_translate_layout_point, wr_translate_layout_size,
        };
        ...
        let (begin_pt, end_pt) = linear_gradient
            .direction
            .to_points(&wr_translate_css_layout_rect(offset_info.rect));
    ...
    fn push_image_background(
        ...
        let background_position =
            calculate_background_position(info, background_position, background_size);
...
mod image {
    ...
    pub(super) fn push_image(
        ...
        use super::{
            ...
            wr_translate_image_rendering, wr_translate_layout_size,
        };
...
mod box_shadow {
    ...
    use azul_core::display_list::{StyleBorderRadius, StyleBoxShadow};
    use azul_css::{BoxShadowClipMode, BoxShadowPreDisplayItem, ColorF, LayoutRect};
    use webrender::api::{
        DisplayListBuilder as WrDisplayListBuilder, LayoutPrimitiveInfo as WrLayoutPrimitiveInfo,
        SpaceAndClipInfo as WrSpaceAndClipInfo,
    };
    ...
    pub(super) fn push_box_shadow(
        ...
        fn translate_shadow_side(
            input: &Option<CssPropertyValue<BoxShadowPreDisplayItem>>,
        ) -> Option<BoxShadowPreDisplayItem> {
    ...
    fn push_box_shadow_inner(
        ...
        use webrender::api::{LayoutPoint, LayoutRect, LayoutVector2D};
...
mod border {
    ...
    use webrender::api::{
        ...
        DisplayListBuilder as WrDisplayListBuilder, LayoutPrimitiveInfo as WrLayoutPrimitiveInfo,
        LayoutSideOffsets as WrLayoutSideOffsets, SpaceAndClipInfo as WrSpaceAndClipInfo,
    };
    ...
    pub(super) fn push_border(
        ...
        if let Some((border_widths, border_details)) =
            get_webrender_border(rect_size, radii, widths, colors, styles)
        {
            builder.push_border(info, space_and_clip, border_widths, border_details);
        }
    }
    ...
    fn get_webrender_border(
        ...
        let border_widths = WrLayoutSideOffsets::new(
            width_top
                .map(|v| v.to_pixels(rect_size.height))
                .unwrap_or(0.0),
            width_right
                .map(|v| v.to_pixels(rect_size.width))
                .unwrap_or(0.0),
            width_bottom
                .map(|v| v.to_pixels(rect_size.height))
                .unwrap_or(0.0),
        ...
        let border_details = WrBorderDetails::Normal(WrNormalBorder {
            top: WrBorderSide {
                color: wr_translate_color_u(color_top.0).into(),
                style: translate_wr_border(style_top, width_top),
            },
            left: WrBorderSide {
                color: wr_translate_color_u(color_left.0).into(),
                style: translate_wr_border(style_left, width_left),
            },
            right: WrBorderSide {
                color: wr_translate_color_u(color_right.0).into(),
                style: translate_wr_border(style_right, width_right),
            },
            bottom: WrBorderSide {
                color: wr_translate_color_u(color_bottom.0).into(),
