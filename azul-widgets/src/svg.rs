use azul_core::{
    ...
    gl::{
        GlApiVersion, GlShader, IndexBufferFormat, Texture, Uniform, VertexAttribute,
...
pub use lyon::{geom::math::Point, path::PathEvent, tessellation::VertexBuffers};
...
impl SvgShader {
    pub fn new(gl_context: Rc<dyn Gl>) -> Self {
        ...
        Self {
            program: GlShader::new(
                gl_context,
                &vertex_source_prefixed,
                &fragment_source_prefixed,
            )
            .unwrap(),
        }
    }
...
fn fill_vertex_buffer_cache<'a>(
    ...
    match gpu_cache.entry(*id) {
        ...
        Vacant(v) => {
            ...
            v.insert(Rc::new(VertexBuffer::new(
                shader.clone(),
                &vbuf,
                &ibuf,
...
impl SvgCache {
    ...
    fn get_fill_vertices_and_indices(
        &self,
        shader: &GlShader,
        id: &SvgLayerId,
    ...
    fn get_stroke_vertices_and_indices(
        &self,
        shader: &GlShader,
        id: &SvgLayerId,
    ) -> Option<Rc<VertexBuffer<SvgVert>>> {
...
impl SampledBezierCurve {
    ...
    pub fn get_text_offsets_and_rotations(
        ...
        for glyph_idx in 0..glyphs.len() {
            ...
            glyph_offsets.push((
                char_bezier_pt.x / SVG_FAKE_FONT_SIZE,
                char_bezier_pt.y / SVG_FAKE_FONT_SIZE,
            ));
            ...
            current_offset =
                start_offset + glyphs.get(glyph_idx + 1).map(|g| g.point.x).unwrap_or(0.0);
        }
...
impl VectorizedFont {
    ...
    pub fn get_fill_vertices(&self, glyphs: &[GlyphInstance]) -> Vec<VertexBuffers<SvgVert, u32>> {
        ...
        glyphs
            .iter()
            .filter_map(|glyph| match borrow_mut.entry(glyph.index) {
    ...
    vertex_buffers
        ...
        .for_each(|(vertex_buf, gid)| {
            ...
            transform_vertex_buffer(
                ...
                gid.point.x / SVG_FAKE_FONT_SIZE,
                gid.point.y / SVG_FAKE_FONT_SIZE,
            );
        });
}
...
impl Svg {
    ...
    pub fn render_svg<T>(
        ...
        let zoom = if self.enable_hidpi {
            ...
            self.zoom
        } * self.multisampling_factor as f32;
        ...
        let pan = (
            pan.0 * self.multisampling_factor as f32,
            pan.1 * self.multisampling_factor as f32,
        );
        ...
        for layer in &self.layers {
            ...
            let fill_vi = match &layer {
                SvgLayerResource::Reference((layer_id, _)) => {
                    svg_cache.get_fill_vertices_and_indices(&shader, layer_id)
                }
                SvgLayerResource::Direct(d) => d.fill.as_ref().map(|f| {
                    Rc::new(VertexBuffer::new(
                        &shader,
                        &f.vertices,
                        &f.indices,
                        IndexBufferFormat::Triangles,
            ...
            let stroke_vi = match &layer {
                SvgLayerResource::Reference((layer_id, _)) => {
                    svg_cache.get_stroke_vertices_and_indices(&shader, layer_id)
                }
            ...
            if let (Some(fill_color), Some(fill_vi)) = (style.fill, fill_vi) {
                let uniforms =
                    build_uniforms(&bbox_size, fill_color, z_index, pan, zoom, &style.transform);
...
fn build_uniforms(
    ...
    vec![
        ...
        Uniform::new(
            "vRotationCenter",
            FloatVec2([layer_rotation_center.x, layer_rotation_center.y]),
        ),
        Uniform::new("vRotationSin", Float(rotation_sin)),
        Uniform::new("vRotationCos", Float(rotation_cos)),
        Uniform::new(
            "vScaleFactor",
            FloatVec2([layer_scale_factor.x, layer_scale_factor.y]),
        ),
        Uniform::new(
            "vTranslatePx",
            FloatVec2([layer_translation.x, layer_translation.y]),
        ),
        // Fragment shader
        Uniform::new(
            "fFillColor",
            FloatVec4([color.r, color.g, color.b, color.a]),
        ),
    ]
}
