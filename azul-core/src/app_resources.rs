...
impl fmt::Debug for GlyphInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "GlyphInfo {{ codepoint: {}, mask: {}, cluster: {} }}",
            self.codepoint, self.mask, self.cluster
        )
    }
}
...
macro_rules! unique_id {
    ($struct_name:ident, $counter_name:ident) => {
        static $counter_name: ::std::sync::atomic::AtomicUsize =
        ...
        impl $struct_name {
            pub fn new() -> Self {
                Self {
                    id: $counter_name.fetch_add(1, ::std::sync::atomic::Ordering::SeqCst),
                }
            }
        }
    };
...
impl AppResources {
    ...
    pub fn add_css_image_id<S: Into<String>>(&mut self, css_id: S) -> ImageId {
        *self
            .css_ids_to_image_ids
            .entry(css_id.into())
            .or_insert_with(|| ImageId::new())
    }
