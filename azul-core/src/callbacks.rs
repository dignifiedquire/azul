...
impl DefaultCallbackId {
    pub fn new() -> Self {
        DefaultCallbackId {
            id: LAST_DEFAULT_CALLBACK_ID.fetch_add(1, Ordering::SeqCst),
        }
    }
}
...
impl<'a, T> DefaultCallbackInfoUnchecked<'a, T> {
    pub unsafe fn invoke_callback<U: Sized + 'static>(
        self,
...
pub type IFrameCallbackTypeUnchecked<T> =
    fn(IFrameCallbackInfoUnchecked<T>) -> IFrameCallbackReturn<T>;
...
impl<'a, 'b, T: 'b> IFrameCallbackInfoUnchecked<'a, 'b, T> {
    pub unsafe fn invoke_callback<U: Sized + 'static>(
        self,
        callback: IFrameCallbackType<T, U>,
    ) -> IFrameCallbackReturn<T> {
        let casted_value: &mut U = self.ptr.cast();
...
impl<'a, T: 'a> Iterator for ParentNodesIterator<'a, T> {
    ...
    fn next(&mut self) -> Option<(DomId, NodeId)> {
        let parent_node_id =
            self.ui_state[&self.current_item.0].dom.arena.node_layout[self.current_item.1].parent?;
