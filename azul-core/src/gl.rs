    ...
    pub fn bind(&self, shader: &GlShader) {
        ...
        for vertex_attribute in self.fields.iter() {
            let attribute_location = vertex_attribute
                ...
                .map(|ll| ll as i32)
                .unwrap_or_else(|| {
                    gl_context.get_attrib_location(shader.program_id, &vertex_attribute.name)
                });
...
impl<T: VertexLayoutDescription> Drop for VertexBuffer<T> {
    fn drop(&mut self) {
        self.gl_context
            .delete_buffers(&[self.vertex_buffer_id, self.index_buffer_id]);
    }
}
    ...
    pub fn draw<T: VertexLayoutDescription>(
            ...
            for uniform in uniforms.iter() {
                if !uniform_locations.contains_key(&uniform.name) {
                    uniform_locations.insert(
                        uniform.name.clone(),
                        gl_context.get_uniform_location(self.program_id, &uniform.name),
                    );
                }
            }
        ...
        for (vi, uniforms) in buffers {
            ...
            gl_context.draw_elements(
                vertex_buffer.index_buffer_format.get_gl_id(),
                vertex_buffer.index_buffer_len as i32,
                INDEX_TYPE,
                0,
            );
        }
