...
use crate::{
    ...
    ui_state::{ActiveHover, HoverGroup, UiState},
};
...
pub fn matches_html_element<'a, T>(
    ...
    for (content_group, reason) in CssGroupIterator::new(&css_path.selectors) {
        ...
        let current_selector_matches =
            selector_group_matches(&content_group, &html_node_tree[cur_node_id]);
...
pub fn match_dom_selectors<T>(
    ...
    let mut styled_nodes = ui_state
        .dom
        .arena
        .node_data
        .transform(|_, node_id| StyledNode {
            css_constraints: css
                .rules()
                .filter(|rule| {
                ...
                .flat_map(|matched_rule| {
                    matched_rule
                        .declarations
                        .iter()
                        .map(|declaration| (declaration.get_type(), declaration.clone()))
                })
                .collect(),
        });
    ...
    for (_depth, parent_id) in non_leaf_nodes {
        let inherited_rules: Vec<CssDeclaration> = styled_nodes[parent_id]
            .css_constraints
            .values()
            .filter(|prop| prop.is_inheritable())
            .cloned()
            .collect();
...
pub fn collect_hover_groups(css: &Css) -> BTreeMap<CssPath, HoverGroup> {
    ...
    css.rules()
        .filter_map(|rule_block| {
            ...
            let css_path = CssPath {
                selectors: rule_block
                    .path
                    .selectors
                    .iter()
                    .cloned()
            ...
            let hover_group = HoverGroup {
                affects_layout: rule_block
                    .declarations
                    .iter()
                    .any(|hover_rule| hover_rule.can_trigger_relayout()),
                active_or_hover,
            };
        ...
        match selector {
            ...
            Id(id) => {
                if !html_node
                    ...
                    .get_ids()
                    .iter()
                    .any(|html_id| html_id.equals_str(id))
                {
                    return false;
                }
            }
            PseudoSelector(CssPathPseudoSelector::First) => {
                ...
                if html_node.index_in_parent != 1 {
                    return false;
                }
            }
            ...
            PseudoSelector(CssPathPseudoSelector::NthChild(x)) => match *x {
                Number(value) => {
                    if html_node.index_in_parent != value {
                        return false;
                    }
                }
                Even => {
                    if html_node.index_in_parent % 2 == 0 {
                        return false;
                    }
                }
                Odd => {
                    if html_node.index_in_parent % 2 == 1 {
                        return false;
                    }
                }
                Pattern { repeat, offset } => {
...
pub fn update_focus_from_callbacks<'a, T: 'a>(
    ...
    match new_focus_target {
        ...
        FocusTarget::Path(css_path) => {
            if let Some(new_focused_node_id) = html_node_tree.linear_iter().find(|node_id| {
                matches_html_element(&css_path, *node_id, &node_hierarchy, &html_node_tree)
            }) {
...
fn test_css_group_iterator() {
    ...
    assert_eq!(
        it.next(),
        Some((
            vec![&Type(NodeTypePath::Div), &Class("content".into()),],
    ...
    assert_eq!(
        it.next(),
        Some((
            vec![&Id("id_test".into()), &Class("new_class".into()),],
    ...
    // Test single class
    let selectors_2 = vec![Class("content".into())];
