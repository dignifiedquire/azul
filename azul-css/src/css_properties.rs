...
impl fmt::Display for BoxShadowPreDisplayItem {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            f,
            "{} {} {} {} {}",
            self.offset[0], self.offset[1], self.blur_radius, self.spread_radius, self.color,
        )
    }
}
...
impl Default for ScrollbarInfo {
    fn default() -> Self {
        ScrollbarInfo {
            ...
            track: RectStyle {
                background: Some(CssPropertyValue::Exact(StyleBackgroundContent::Color(
                    ColorU {
                        r: 241,
                        g: 241,
                        b: 241,
...
impl RectStyle {
    ...
    pub fn has_box_shadow(&self) -> bool {
        self.box_shadow_left
            .and_then(|bs| bs.get_property().map(|_| ()))
            .is_some()
            || self
                .box_shadow_right
                .and_then(|bs| bs.get_property().map(|_| ()))
    ...
    pub fn has_border(&self) -> bool {
        self.border_left_style
            .and_then(|bs| bs.get_property_or_default())
            .is_some()
            || self
                .border_right_style
                .and_then(|bs| bs.get_property_or_default())
...
impl RectLayout {
    pub fn is_horizontal_overflow_visible(&self) -> bool {
        self.overflow_x.map(|css_prop| {
            css_prop
                .get_property()
                .map(|overflow| overflow.is_overflow_visible())
                .unwrap_or_default()
        }) == Some(true)
    }

    pub fn is_vertical_overflow_visible(&self) -> bool {
        self.overflow_y.map(|css_prop| {
            css_prop
                .get_property()
                .map(|overflow| overflow.is_overflow_visible())
                .unwrap_or_default()
